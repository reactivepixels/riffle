<!--
  A page using useRiffle from @rpxl/riffle/vue with the directives, server
  rendered by Nuxt. `state` and `activeIndex` come straight from useRiffle's
  reactive snapshot, so the "N / 8" readout and the active film's title below
  are already correct in the HTML Nuxt's server sends, before any hydration.
-->
<template>
  <main class="page">
    <div class="stage">
    ...
    <div class="meta">
      <!-- data-active-title lets check:ssr scope its title assertion to the
           ...
           whole page: every card's own title renders unconditionally above,
           so a plain page-wide substring search cannot tell an active film
           rendered correctly from one hardcoded wrong. -->
      <h1 class="title" data-active-title>{{ active.title }}</h1>
    ...
    <div class="controls">
      ...
      <span class="readout">{{ activeIndex + 1 }} / {{ films.length }}</span>
      <button type="button" class="control" aria-label="Next film" @click="next">
        <span aria-hidden="true">&#8250;</span>
      </button>
    </div>
  </main>
...
// CSS alone. `window` does not exist during SSR, so the server always
// sends the desktop size; a narrow-viewport client corrects it once in
// onMounted, after hydration (not during it, so this is a same-tick
// reactive update, not a hydration mismatch).
 ...
 * breakpoint. Desktop (the rules above) is unchanged; this query only
 * ever narrows them.
 */
