/**
 * Invented films for the poster stack, shared by the programmatic-control
 * and forms-in-cards recipe pages in this app. Each gets a two-stop CSS
 * gradient instead of artwork: there is no licensed poster art to show, and
 * a gradient reads as a poster silhouette without pretending to be a photo.
 * A copy of vue-movie-stack's own films.ts: that example keeps its own for
 * its main demo, so this app carries its own rather than reaching across
 * examples for data.
 */
