 ...
 * inside the threshold window for that whole window: every render that
 ...
 * renders instead of exactly once. The caller is expected to hold
 * `isLoading` true from the moment it starts a fetch until that fetch's
 * cards are appended.
 *
 * Pure and framework free: it decides from plain numbers, so it is tested
 * on its own, with no component mounted.
 */
